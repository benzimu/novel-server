...
RUN apk upgrade --update && \
    apk add wget python python-dev py-pip musl-dev gcc py-curl py-mysqldb openssl-dev libxml2-dev libxslt-dev libffi-dev libxml2 libxslt && \
    pip install --no-cache-dir -r $BASE_WORK/novel-server/novel/requirements.txt && \
    printf "novel.path=$BASE_WORK/novel-server/novel\nnovel.scrapy.path=python" > $BASE_WORK/novel-server/src/main/resources/properties/novel.properties && \
